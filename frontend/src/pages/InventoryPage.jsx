...
export default function InventoryPage() {
  ...
  // FIX: backend field is 'cantidad', not 'stockActual'
  const totalValue = items.reduce((total, item) => total + Number(item.cantidad || 0), 0);

  function openCreate() { setSelectedItem(null); setShowForm(true); }
  function openEdit(item) { setSelectedItem(item); setShowForm(true); }
  ...
  if (loading) return <Loading text="Cargando inventario..." />;
    ...
    <>
      ...
      <div className="table-card">
        <div className="table-toolbar">
          <Form.Control value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre o categoría..." />
        </div>
            ...
            <Table hover>
              <thead>
                <tr>
                  <th>Insumo</th>
                  <th>Tipo</th>
                  <th>Stock Actual</th>   {/* FIX: shows 'cantidad' from backend */}
                  <th>Stock Mínimo</th>
                  <th>Unidad</th>
              ...
              <tbody>
                {filteredItems.map((item) => {
                  // FIX: use 'cantidad' (backend field), not 'stockActual' (doesn't exist)
                  const stockActual = item.cantidad ?? 0;
                  const low = stockActual <= Number(item.stockMinimo || 0);

                  return (
                    <tr key={item.id} className={low ? "table-danger-soft" : ""}>
                      <td><strong>{item.nombre}</strong></td>
                      <td>
                        <Badge bg="light" text="dark">{item.tipo}</Badge>
                      </td>
                      <td className={low ? "text-danger fw-bold" : ""}>{stockActual}</td>
